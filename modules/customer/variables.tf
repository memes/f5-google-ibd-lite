...
variable "address" {
  ...
  description = <<-EOD
  The public IP address to assign to the GCP HTTP(S) Load Balancer that will be
  the ingress for customer requests.
  EOD
}
...
variable "domain_name" {
  ...
  description = <<-EOD
  The public DNS name that will be used to configure Customer host matching in
  URL map.
  EOD
}
...
variable "sse" {
  type = string
  validation {
    condition     = can(cidrhost(format("%s/32", var.sse), 0)) || can(regex("^(?:[a-z0-9][a-z0-9-]{0,61}[a-z0-9]\\.)+[a-z]{2,63}$", var.sse))
    error_message = "The sse variable must be a valid IP address or DNS name."
  }
  description = <<-EOD
  The public IP address or public DNS name for the SSE ingress that will inspect
  and process traffic sent to it.
  EOD
}
...
variable "certificate_map" {
  ...
  the Customer ingress load balancer.
  EOD
}
